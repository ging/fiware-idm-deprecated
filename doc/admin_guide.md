...
The recommended way of installing the IdM is through the installation tools. You can find the documentation on how to use this tools [here](doc/introduction.md/#def-build). 

In case you would rather install it step by step, this guide will cover all the steps required to successfully install the IdM,  and as a reference of the installation scripts tasks.
...
**IMPORTANT NOTE**: As the Django documentation states: DO NOT USE THIS SERVER IN A PRODUCTION SETTING. It has not gone through security audits or performance tests. For a production setting, follow the [production setup guide](http://fiware-idm.readthedocs.org/en/latest/setup)
...
Run the following commands to create the database. By default it will create a SQLite database. If you want to use a MySQL database (recommended for production) follow the configuration instructions in the [production setup guide](http://fiware-idm.readthedocs.org/en/latest/setup/)
